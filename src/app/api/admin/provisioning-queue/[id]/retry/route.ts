...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!challenge) return NextResponse.json({ error: "Not found or already allocated" }, { status: 404 });

  const sizeMatch = challenge.challenge_id.match(/(\d+)k/i);
  const accountSize = sizeMatch ? Number(sizeMatch[1]) * 1000 : null;
  if (!accountSize) return NextResponse.json({ error: "Could not determine account size" }, { status: 500 });
