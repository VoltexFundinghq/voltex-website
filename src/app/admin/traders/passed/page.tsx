...
function phaseLabel(phase: number): string {
  if (phase === 1) return "Passed Phase 1";
  if (phase === 2) return "Passed Phase 2";
  return `Phase ${phase}`;
}

export default async function PassedTradersPage() {
  const traders = await getTradersByStatus("passed");

  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">Passed Traders</h1>
        <p className="mt-1 text-sm text-zinc-500">{traders.length} trader{traders.length === 1 ? "" : "s"} who completed evaluation, awaiting or moved to funded</p>
      </div>

      {traders.length === 0 ? (
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-12 text-center">
          <p className="text-zinc-500">No passed traders yet.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10 bg-white/[0.03] text-left text-xs uppercase tracking-wide text-zinc-500">
                ...
                <th className="px-4 py-3 font-medium">Passed On</th>
              </tr>
            </thead>
            <tbody>
              {traders.map((t) => (
