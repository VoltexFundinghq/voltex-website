...
const PIE_COLORS = ["#D4AF37", "#3b82f6", "#ef4444", "#f59e0b", "#22c55e", "#71717a", "#8b5cf6"];

function EmptyChart() {
  return <div className="flex h-[180px] items-center justify-center text-sm text-zinc-600">No data yet.</div>;
}

export default function ManualReviewCharts({ data }: { data: ChartsData }) {
  const hasCategory = data.byCategory.some((d) => d.count > 0);
  const has30Day = data.last30Days.some((d) => d.count > 0);
  const hasPriority = data.priorityDistribution.some((d) => d.count > 0);

  return (
    <div className="grid gap-6 md:grid-cols-2">
      <div className="rounded-xl border border-white/10 bg-white/[0.02] p-5">
        <h3 className="mb-3 text-sm font-semibold text-white">Open Reviews by Category</h3>
        {hasCategory ? (
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.02] p-5">
        <h3 className="mb-3 text-sm font-semibold text-white">Reviews — Last 30 Days</h3>
        {has30Day ? (
          <ResponsiveContainer width="100%" height={180}>
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.02] p-5 md:col-span-2">
        <h3 className="mb-3 text-sm font-semibold text-white">Priority Distribution</h3>
        {hasPriority ? (
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={data.priorityDistribution} layout="vertical" margin={{ left: 20 }}>
              ...
              <YAxis type="category" dataKey="priority" stroke="rgba(255,255,255,0.3)" fontSize={11} tickLine={false} axisLine={false} width={80} />
              <Tooltip contentStyle={{ background: "#0a0a0a", border: "1px solid rgba(212,175,55,0.2)", borderRadius: "8px" }} />
