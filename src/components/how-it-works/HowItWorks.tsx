...
const steps = [
  {
    ...
    description:
      "Choose your account size from ₦200,000 to ₦800,000 and begin your evaluation.",
  },
  {
    icon: TrendingUp,
    title: "Reach 10% Profit Target",
    description:
      "Trade responsibly within the 10% daily and 20% max drawdown rules.",
  },
  {
    icon: Trophy,
    title: "Get Funded",
    description:
      "Receive your funded account and earn up to 90% profit split.",
  },
];
...
function TravelingLight({ delay = 0, vertical = false }: { delay?: number; vertical?: boolean }) {
  return (
    <div
      className={
        vertical
          ? "relative h-full w-[3px] overflow-hidden rounded-full bg-[#D4AF37]/20"
          : "relative h-[3px] w-full overflow-hidden rounded-full bg-[#D4AF37]/20"
      }
    >
      <motion.div
        className={
          vertical
            ? "absolute inset-x-0 h-1/2 bg-gradient-to-b from-transparent via-[#F5D573] to-transparent"
            : "absolute inset-y-0 w-1/2 bg-gradient-to-r from-transparent via-[#F5D573] to-transparent"
        }
        style={{ filter: "drop-shadow(0 0 8px rgba(212,175,55,0.9))" }}
        animate={
          vertical
            ? { top: ["-50%", "120%"], opacity: [0, 1, 1, 0] }
            : { left: ["-50%", "120%"], opacity: [0, 1, 1, 0] }
        }
        transition={{
          duration: 2.6,
          repeat: Infinity,
...
function StepCard({
  ...
  return (
    <motion.div
      ...
      className="group relative flex w-full flex-col items-center justify-center rounded-2xl border border-[#D4AF37]/20 bg-white/[0.02] px-6 py-[25px] text-center backdrop-blur-xl transition-colors duration-300 hover:border-[#D4AF37]/70"
    ...
    >
      ...
      <p className="mx-auto mt-2.5 max-w-[210px] text-sm leading-7 text-zinc-400">
        {step.description}
      </p>
    </motion.div>
  );
  ...
  return (
    <section className="relative overflow-hidden bg-black pt-10 pb-8">
      ...
      {particles.map((p, i) => (
        <motion.div
          key={i}
          className="pointer-events-none absolute rounded-full bg-[#D4AF37] blur-[1px]"
      ...
      <div className="relative mx-auto max-w-[1440px] px-8">
        ...
        >
          <p className="mb-1.5 text-xs font-semibold uppercase tracking-[0.3em] text-[#D4AF37]">
            How It Works
          </p>
          <h2 className="text-3xl font-extrabold leading-tight text-white">
            Become a Funded Trader in <span className="text-[#D4AF37]">3 Simple Steps</span>
          </h2>
          <p className="mt-2 text-sm leading-6 text-zinc-300">
            Getting funded is simple. Purchase a challenge, prove your consistency, and receive a funded account.
          </p>
        </motion.div>

        {/* Steps — desktop: 3 equal cards with animated connector */}
          ...
          {steps.map((step, i) => (
            <div key={step.title} className="flex items-stretch">
              <div className="lg:w-[390px]">
                <StepCard step={step} delay={0.15 + i * 0.35} />
              </div>

              {i < steps.length - 1 && (
                <div className="flex w-24 flex-shrink-0 items-center justify-center">
                  <TravelingLight delay={0.15 + i * 0.35 + 0.2} />
                </div>
              )}
            </div>
          ))}
        ...
        {/* Steps — tablet: 2 + 1 grid, no connector line */}
        <div className="hidden sm:grid sm:grid-cols-2 sm:gap-6 lg:hidden">
          {steps.map((step, i) => (
            <div
              key={step.title}
              className={i === steps.length - 1 ? "sm:col-span-2 sm:flex sm:justify-center" : ""}
            >
              <div className={i === steps.length - 1 ? "sm:w-[calc(50%-12px)]" : "w-full"}>
                <StepCard step={step} delay={0.1 + i * 0.2} />
              </div>
            </div>
          ))}
        </div>

        {/* Steps — mobile: 1 per row, vertical connector */}
        <div className="flex flex-col items-stretch sm:hidden">
          {steps.map((step, i) => (
            <div key={step.title}>
              <StepCard step={step} delay={0.1 + i * 0.25} />

              {i < steps.length - 1 && (
                <div className="mx-auto flex h-6 w-[3px] items-center justify-center">
                  <TravelingLight delay={0.1 + i * 0.25 + 0.15} vertical />
                </div>
              )}
            </div>
          ))}
        ...
        <div className="relative mx-auto mt-8 max-w-[640px]">
          ...
          <motion.div
            ...
            className="relative rounded-2xl border border-[#D4AF37]/30 bg-white/[0.03] p-7 text-center backdrop-blur-xl"
            style={{ boxShadow: "0 15px 40px rgba(212,175,55,0.12)" }}
          >
            <div className="relative mx-auto flex h-16 w-16 items-center justify-center">
              <div className="absolute inset-0 rounded-full bg-[#D4AF37]/40 blur-2xl" />
              <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-[#D4AF37]/10 ring-1 ring-[#D4AF37]/40">
            ...
            <div className="mx-auto mt-4 flex max-w-sm flex-col gap-1.5 text-left">
              <div className="flex items-start gap-2">
                ...
                <span className="text-xs leading-5 text-zinc-300">
                  Receive up to 90% profit split.
                </span>
              </div>
              <div className="flex items-start gap-2">
                <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-[#D4AF37]" />
                <span className="text-xs leading-5 text-zinc-300">
                  Withdraw eligible profits within 24 hours.
                </span>
              </div>
            </div>
          </motion.div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {perks.map((perk, i) => (
            <motion.div
              ...
              className="group flex items-center gap-3.5 rounded-xl border border-[#D4AF37]/20 bg-white/[0.02] p-4 backdrop-blur-xl transition-colors duration-300 hover:border-[#D4AF37]/60"
              style={{ boxShadow: "0 0 18px rgba(212,175,55,0.05)" }}
            >
              <div className="relative flex h-12 w-12 flex-shrink-0 items-center justify-center">
                ...
                <div className="relative flex h-12 w-12 items-center justify-center rounded-lg bg-[#D4AF37]/10 text-[#D4AF37] ring-1 ring-[#D4AF37]/30 transition-colors duration-300 group-hover:bg-[#D4AF37] group-hover:text-black">
        ...
        >
          ...
          <p className="mt-1.5 text-sm text-zinc-400">
            Choose your challenge below and begin your journey today.
          </p>
        </motion.div>
