...
function StepCard({ step }: { step: (typeof steps)[number] }) {
  ...
  return (
    <motion.div whileHover={{ y: -5, scale: 1.02 }} className="group relative flex w-full flex-col items-center justify-center rounded-2xl border border-[#D4AF37]/20 bg-white/[0.02] px-6 py-[25px] text-center backdrop-blur-xl transition-colors duration-300 hover:border-[#D4AF37]/70">
...
function PerkCard({ perk }: { perk: (typeof perks)[number] }) {
  ...
  return (
    <motion.div whileHover={{ y: -5, scale: 1.03 }} className="group flex h-full items-center gap-3.5 rounded-xl border border-[#D4AF37]/20 bg-white/[0.02] p-4 backdrop-blur-xl transition-colors duration-300 hover:border-[#D4AF37]/60">
      <div className="relative flex h-12 w-12 flex-shrink-0 items-center justify-center">
        ...
        <div className="relative flex h-12 w-12 items-center justify-center rounded-lg bg-[#D4AF37]/10 text-[#D4AF37] ring-1 ring-[#D4AF37]/30 transition-colors duration-300 group-hover:bg-[#D4AF37] group-hover:text-black"><Icon className="h-5 w-5" /></div>
      </div>
    ...
    <section className="relative overflow-hidden bg-black pt-10 pb-8">
      ...
      <div className="relative mx-auto max-w-[1440px] px-8">
        ...
        <div className="relative hidden lg:flex lg:items-stretch lg:justify-center">
          {steps.map((step, i) => (
            <div key={step.title} className="flex items-stretch">
              <div className="lg:w-[390px]"><StepCard step={step} /></div>
        ...
        <div className="hidden sm:grid sm:grid-cols-2 sm:gap-6 lg:hidden">
          {steps.map((step, i) => (
            <div key={step.title} className={i === steps.length - 1 ? "sm:col-span-2 sm:flex sm:justify-center" : ""}>
              <div className={i === steps.length - 1 ? "sm:w-[calc(50%-12px)]" : "w-full"}><StepCard step={step} /></div>
            </div>
          ))}
        </div>

        <div className="flex flex-col items-stretch gap-6 sm:hidden">
          {steps.map((step, i) => (
            <div key={step.title}>
              <StepCard step={step} />
              {i < steps.length - 1 && (<div className="mx-auto mt-6 flex h-6 w-[3px] items-center justify-center"><TravelingLight delay={i * 0.08 + 0.08} vertical /></div>)}
            </div>
          ))}
        </div>
        ...
        <div className="relative mx-auto mt-8 max-w-[640px]">
          ...
          <div className="relative rounded-2xl border border-[#D4AF37]/30 bg-white/[0.03] p-7 text-center backdrop-blur-xl">
        ...
        <div className="relative mt-8 overflow-hidden sm:hidden" style={{ WebkitMaskImage: "linear-gradient(to right, transparent, black 8%, black 92%, transparent)", maskImage: "linear-gradient(to right, transparent, black 8%, black 92%, transparent)" }}>
          <div className="flex w-max animate-[voltex-marquee-hiw_18s_linear_infinite]">
            {[...perks, ...perks].map((perk, i) => {
              ...
              return (
                <div key={i} className="flex flex-shrink-0 items-center gap-3 pr-10">
                  ...
                  <p className="whitespace-nowrap text-base font-semibold text-white">{perk.label}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="mt-8 hidden sm:grid sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
          {perks.map((perk) => (<PerkCard key={perk.label} perk={perk} />))}
        </div>
